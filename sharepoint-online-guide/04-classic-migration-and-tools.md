...
| **SharePoint 2013 ワークフロー** | 非推奨。**2026年4月に完全廃止予定** 〔S13〕 | **Power Automate** |
| **InfoPath フォーム** | サポート終了フェーズ（**2026年7月14日でサポート終了予定**） 〔S13〕 | **Power Apps／Forms／Lists フォーム** |
| クラシック Web パーツ／旧カスタマイズ（サーバーサイド等） | モダンで非対応のものが多い | **SPFx**、モダン Web パーツ、Power Platform |

> ⚠️ **【重要】ワークフロー・フォームの棚卸しを最優先で**
> 「ファイルを移せば終わり」と思っていると、**業務を回している 2013 ワークフローや InfoPath フォームが移行後に動かない**という事故が起きます。廃止スケジュール（2026年）が迫っているため、**現行クラシック環境で何が動いているかの棚卸し**を移行計画の最初に行ってください 〔S13〕。
...
- **デメリット**：ファイルサイズ上限（250GB 目安）等の制約。**複数マシンでの並列分散（マルチエージェント）に非対応**。各ジョブを個別に監視する必要があり一元ダッシュボードがない
- **費用**：**無償**
...
- **デメリット**：ファイルサイズ上限（250GB 目安）。純正ゆえ、権限マッピング等の柔軟性はサードパーティに劣る面がある
- **費用**：**無償**
...
- **メリット**：検出・スキャンは無償。権限・バージョン・機密ラベル等を保持する**高忠実度（Full Fidelity）マッピング**、TB 級の大容量対応、**ガバナンス機能**（権限リスクの検出・是正）が強い 〔V2〕
...
| --- | --- | --- | --- | --- | --- |
| **SPMT** | Microsoft | **無償** | ファイル共有、SP Server 2010-2019 | 無料、モダン構造へ変換可 | 250GB上限、並列分散不可、個別監視 |
| **Migration Manager** | Microsoft | **無償** | ファイル共有、オンプレSP、各種クラウド | エージェント並列分散、一元管理、大規模向き | 250GB上限、権限マッピングの柔軟性は限定的 |
...
- **2013 ワークフロー（2026年4月廃止予定）・InfoPath（2026年7月サポート終了予定）**を最優先で棚卸しし、Power Automate / Power Apps へ作り替える 〔S13〕。
