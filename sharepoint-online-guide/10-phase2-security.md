...
> - **［要追加ライセンス］** … 別途の購入・契約が必要（カッコ内が対象。E5／E5 Compliance／Microsoft Entra ID P2 等）

## この章でできること（全体像）

まず「何ができるか」を俯瞰します。左側が **［E3標準］（追加費用なし）**、右側が **［要追加ライセンス］** で実現できる範囲です。詳細は §1 以降で説明します。

```mermaid
%%{init:{'theme':'neutral'}}%%
flowchart LR
    P(["フェーズ2：セキュリティ<br/>できることの全体像"])
    P --> E3["E3標準（追加費用なし）"]
    P --> AD["要追加ライセンス"]
    E3 --> e1["操作の記録を残す<br/>（監査ログ）"]
    E3 --> e2["ファイルに社外秘の札を付け守る<br/>（手動ラベル・暗号化）"]
    E3 --> e3["機微情報の外部共有をブロック<br/>（DLP：ファイル）"]
    E3 --> e4["入口をMFAで固める<br/>（条件付きアクセス）"]
    AD --> a1["ラベルを自動で付与（E5）"]
    AD --> a2["チャット本文・PCのUSBを制御（E5）"]
    AD --> a3["監査ログを長期保持（E5）"]
...
| 機能 | できること | 追加費用 |
| --- | --- | --- |
| 監査ログ（Standard・180日） | 操作の記録を残し調査する | **E3標準**（追加費用なし）〔S24〕 |
| 秘密度ラベル（手動付与・暗号化） | ファイルに「社外秘」等を付けて守る | **E3標準**（追加費用なし）〔S18〕 |
| DLP（SharePoint/OneDrive/Exchange） | 機微情報の外部共有をブロック | **E3標準**（追加費用なし）〔S22〕 |
| 条件付きアクセス（基本）／MFA | 入口を MFA・デバイス条件で固める | **E3標準**（Entra P1 は E3 内包）〔S27〕 |
| 保持（手動・場所ベース） | 規程に沿って保持・自動削除 | **E3標準**（追加費用なし）〔S25〕 |
| 秘密度ラベルの**自動**付与 | 中身を判定して自動でラベル | **要追加ライセンス**（E5／E3＋E5 Compliance）〔S19〕 |
| DLP（**Teams チャット本文**） | チャット内の機微情報を検知 | **要追加ライセンス**（E5）〔S22〕 |
| **エンドポイント DLP**（PC 操作） | USB コピー・印刷・アップロードを制限 | **要追加ライセンス**（E5 相当）〔S23〕 |
| 監査ログ **Premium**（1年保持等） | 長期保持・高付加価値イベント | **要追加ライセンス**（E5／E5 Compliance）〔S24〕 |
| 保持の**内容ベース自動適用** | 分類子・機微情報で自動ラベル | **要追加ライセンス**（E5）〔S25〕 |
| eDiscovery（Premium） | 訴訟・調査向けの高度な開示 | **要追加ライセンス**（E5 系）〔S25〕 |
| **リスクベース**条件付きアクセス | サインインリスクで動的制御 | **要追加ライセンス**（E5＝Entra P2）〔S27〕 |
| **アクセスレビュー**（ゲスト棚卸し） | ゲストを自動で定期見直し | **要追加ライセンス**（E5 相当＝Entra P2）〔S28〕 |
...
>
> **補足**：上位機能は原則、その機能の恩恵を受ける**利用者側**に上位ライセンスが必要です（例：自動ラベル付けは対象文書を持つ利用者に E5 相当）。「管理者だけ E5」で済むとは限りません。機能ごとに一次情報で確認してから調達してください〔S19〕。ライセンス条件は改定されるため、契約前に「Microsoft Purview サービスの説明」等の一次情報で最終確認を。
