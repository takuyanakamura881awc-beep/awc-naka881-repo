...
フェーズ1で「正しい土台（構成・棲み分け・権限）」ができたら、次はその上に**情報保護の網**をかけます。フェーズ2では、Office ファイルへの「社外秘／極秘」等の**秘密度ラベル**、情報漏えい防止（**DLP**）、そして監査向けの**ログ機能（監査）**の有効化を扱います。

> ⚠️ **【最重要・ライセンス前提】このフェーズは E3 単独では完結しません**
> フェーズ1は E3 の範囲でほぼ実現できましたが、**フェーズ2の多くの機能は E5／E5 Compliance／Microsoft Entra ID P2 等の追加ライセンスを必要**とします。本章では各機能に必要ライセンスを明記します。まず「E3 で今すぐできること」から着手し、E5 相当が要る施策は投資判断とセットで段階導入するのが現実的です。
>
> なお、ここに記載するライセンス切り分けは Microsoft の一般公開情報に基づく整理です。**契約・実装前に必ず最新の「Microsoft Purview サービスの説明」等の一次情報でご確認ください**（機能別ライセンス表は改定されます）。

## 0. ライセンス早見表（フェーズ2でE3のまま出来ること / 追加が要ること）

| 施策 | E3 の範囲でできる | 追加ライセンスが必要 |
| --- | --- | --- |
| 秘密度ラベルの**手動**付与・暗号化 | ○（E3）〔S18〕 | — |
| 秘密度ラベルの**自動**付与 | ✕ | **E5** または **E3＋E5 Compliance** 〔S19〕 |
| サイト/Teams/グループの**コンテナラベル** | 基本設定は可（要ライセンス確認）〔S20〕 | 一部は上位ライセンス／Entra 連携が前提 |
| DLP（SharePoint/OneDrive/Exchange 経由のファイル） | ○（E3）〔S22〕 | — |
| DLP（**Teams チャット/チャネル本文**） | ✕ | **E5** 〔S22〕 |
| **エンドポイント DLP**（PC の USB コピー制限等） | ✕ | **E5 相当** 〔S23〕 |
| 監査ログ **Standard**（保持 180 日） | ○（E3）〔S24〕 | — |
| 監査ログ **Premium**（保持 1 年・高付加価値イベント） | ✕ | **E5／E5 Compliance** 〔S24〕 |
| 保持ポリシー・保持ラベル（手動・場所ベース） | ○（E3）〔S25〕 | 内容ベースの**自動適用**（分類子・機微情報トリガー）は **E5** |
| eDiscovery（Premium） | ✕ | **E5 系** 〔S25〕 |
| **条件付きアクセス**（基本） | ○（Entra ID P1＝E3 に内包）〔S27〕 | — |
| **リスクベース条件付きアクセス** | ✕ | **Entra ID P2＝E5 に内包** 〔S27〕 |
| **アクセスレビュー**（ゲスト棚卸し） | ✕ | **Entra ID Governance／P2** 〔S28〕 |
| セキュリティ既定値（MFA 一括強制） | ○（無償・全 SKU）〔S27〕 | — |

> 💡 **【補足】機能ごとに「誰にライセンスが要るか」を確認する**
> 上位機能は、原則として**その機能の恩恵を受ける利用者側**に上位ライセンス（E5／E5 Compliance 等）が必要です。たとえば**自動ラベル付けは、対象コンテンツを持つ利用者に E5 相当が必要**で、「管理者だけ E5、利用者は E3」で成立するとは限りません（一部の純粋な管理系機能のみ管理者ライセンスで足りる場合があります）。全社一律 E5 化を避けたい場合でも、**どの利用者に何のライセンスが必要か**を機能ごとに一次情報で確認してから調達してください（過少ライセンスはコンプライアンス上の齟齬を招きます）〔S19〕。
