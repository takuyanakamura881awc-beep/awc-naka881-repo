...
> 💡 **【補足】「管理者だけE5」という考え方**
> 自動ラベル付けなど一部の機能は、**設定・実行する管理者アカウントにのみ上位ライセンスが必要**で、一般利用者は E3 のまま恩恵を受けられる構成があります 〔S19〕。全社員を E5 にせずとも一部だけ引き上げて実現できる余地があるため、要件に応じて最小コストの組み合わせを検討します（要ライセンス条件確認）。
...
- **保持ポリシー／保持ラベル**：「経理文書は7年保持」「退職者データは一定期間後に削除」等、法令・社内規程に沿った保持・削除を自動化。**概ね E3 で可能** 〔S25〕。
